...
- [x] 已在父工程补充 `modernization-java21-preview` 预备 profile，作为后续 Java 21 升级入口
- [x] 已在父工程统一 `compiler/resources/surefire` 的基础插件管理，降低后续 Boot 3 / Java 21 升级时的分散改动

当前已确认的 Boot 3 / Java 21 兼容阻塞点：

- [ ] `market-data-service` 的 Redis 序列化仍使用 `ObjectMapper.enableDefaultTyping`，后续需要切到安全且兼容 Jackson 新版本的写法
- [ ] `trend-trading-backtest-service` 仍保留 `OpenFeign` 入口，后续需要切到 `Spring HTTP Service Clients`
- [ ] `market-data-service` 以及部分回测调用链仍使用 `RestTemplate`，后续需要评估迁到 `RestClient` 或 `WebClient`
- [ ] `trend-trading-backtest-view` 仍保留 `bootstrap.yml / bootstrap-nacos.yml` 配置加载方式，后续需要收口到更符合 Boot 3 习惯的配置结构
- [ ] 主线模块虽然已切到 `Nacos`，但仍残留 `@EnableDiscoveryClient` 等旧时代显式注解，后续需要在正式升级时一并清理
...
- 将基于 Feign 的服务通信改造为 Spring HTTP Service Clients，并引入 Java 21 + Spring Boot 3 的现代化架构基线。
