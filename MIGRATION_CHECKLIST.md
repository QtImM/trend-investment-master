...
| 前端入口迁移 | 95% | `trend-web` 已成为默认前端入口，剩余主要是后续体验与治理性优化 |
| 本地运行与交接入口 | 98% | 已具备统一启动、状态、验收脚本、完成态说明与 README 运行顺序/功能说明，剩余仅少量持续优化空间 |
| 测试与可观测性收尾 | 88% | 已有统一栈测试、验收脚本测试、回测控制器测试、回测服务层测试与统一监控说明，剩余主要是更细粒度业务覆盖和长期告警治理 |
| 历史资产彻底清理 | 88% | 主构建与主线文档已基本收口，仍有少量历史规划口径可继续清理 |

综合来看，当前迁移主线整体进度约为 **98%**。

如果按“是否已经足够支撑本地开发与调试”来判断，则当前已经达到 **100% 可用态**；  
如果按“是否完成所有治理、测试、说明与历史资产清理”来判断，则仍保留约 **2%** 的后续收尾空间。
