...
按当前主线目标“完成本地可运行、可验收、可调试的迁移闭环”评估，当前进度同步如下：
...
- 把当前“链路已经可用”继续推进到“仓库内有统一运行入口、统一验收入口、统一完成态说明”
- 不再依赖对话记录、零散命令或日志文件来恢复本地环境
- 为“真正意义上的迁移完成态清理”补齐最后一层运行与文档收口

#### 已完成事项

1. 新增了本地链路统一管理脚本
   - 新增 `.tools/local_stack.py`
   - 支持：
     - `start-nacos`
     - `stop-nacos`
     - `start`
     ...
     - `up`
     - `down`
   - 将本地 `Nacos 2.4.3` 与四个核心服务的常用操作统一收口到一个脚本入口

2. 把核心服务的启动差异收敛进了脚本内部
   - `market-data-service` 继续通过仓库内 `Maven 3.9.9` 执行 `spring-boot:run`
   - `trend-trading-backtest-service`
   - `trend-trading-backtest-view`
   - `gateway-service`
     统一通过已构建好的 `jar` 后台启动
   - 同时统一将日志输出落到 `.tools/*.log`

3. 用真实执行验证了统一入口可用
   - 实际执行 `python .tools/local_stack.py status`
   - 实际执行 `python .tools/local_stack.py up`
   - 实际执行 `python .tools/local_stack.py verify`
   - 验证当前脚本已经能在“服务已运行”的情况下稳定识别状态并完成整体验收
...
4. 更新了根说明文档
   ...
   - 不再要求开发者从对话或历史记录中手动拼启动顺序

5. 更新了迁移清单中的当前完成态说明
   - 更新 `MIGRATION_CHECKLIST.md`
   - 新增“当前迁移完成态快照（2026-03-18）”
   - 明确当前已经完成：
     - `Nacos 2.4.3` 收口
...
当前仓库已经不再只是“本地有人知道怎么跑”：

- 本地栈已经有统一的启动、停止、状态、验收入口
- 根说明文档已经收口为当前真实推荐做法
- 迁移清单也已经显式标记当前完成态，而不是继续停留在早期规划口径

这意味着当前项目在运行与说明层面，已经更接近“真正意义上的迁移完成态”，而不是只能靠上下文口口相传维持。

#### 这一步为什么重要

- 如果没有统一运行入口，主链路即使已经打通，也仍然很容易在交接、复现或后续调试时退回到手工恢复状态
- 把脚本和文档一起收口后，当前可用态终于具备了可重复、可交接、可复核的完成态特征
- 这一步也为后续继续清理旧模块与历史说明提供了稳定基线

#### 下一步计划

下一步优先考虑以下动作：

1. 继续清理仓库中仍保留的旧基础设施与旧入口说明，让文档口径彻底与当前主线一致
2. 视需要继续补齐测试与可观测性资产，把“可用”推进到“更适合长期维护”
3. 在运行与说明已经统一的前提下，再决定是否继续做更激进的历史模块退场

### 2026-03-18 - 阶段 87：清理根文档中的历史运行口径

#### 本阶段目标

- 继续把“迁移完成态”从运行脚本收口到根文档层
- 清理 README 和退场方案文档中仍残留的旧端口、多实例、旧模块状态表述
- 让新同学只看根文档时，也能直接获得当前真实主线口径

#### 已完成事项

1. 收口了 README 中的微服务端口说明
   - 更新 `README.md`
   - 将原先的：
   ...
   - 同时把当前统一入口 `gateway-service | 8032` 放到更靠前位置

2. 将第三方样例数据模块降级为“历史/可选模块”说明
   - 更新 `README.md`
   - 单独增加“历史/可选模块端口”小节
   - 明确 `third-part-index-data-project` 当前只是 fixture / mock-provider，主链路默认不要求启动

3. 修正了老基础设施退场方案文档中的过时状态
   - 更新 `infra/transition/LEGACY_INFRA_RETIREMENT_PLAN.md`
   - 将顶部映射表中：
     - `index-hystrix-dashboard`
     - `index-turbine`
     从“待退场”改为“已退场”
   - 去掉与当前仓库状态不一致的“后退场模块”口径
   - 让退场方案文档与当前源码和主构建状态保持一致

#### 当前结果

当前根文档层的主线口径已经比之前更干净：

- README 不再把当前主线说成仍依赖多实例旧端口布局
- `third-part-index-data-project` 不再与核心主链路混写在同一层级
- 老基础设施退场方案也不再保留“明明已经删除却还写着待退场”的状态噪音

这意味着当前仓库不仅代码和运行时已经完成迁移，连最容易误导后续维护者的历史说明也开始被系统性收口。

#### 这一步为什么重要

- 完成态清理不只是让服务能跑，还要让说明文档不会把后续维护者重新带回旧认知
- 如果 README 还保留旧端口和多实例写法，后续调试者很容易误以为必须恢复旧运行模式
- 把这些历史口径清掉后，当前主线终于更接近“代码、运行、文档三层一致”

#### 下一步计划

下一步优先考虑以下动作：

1. 继续校对其余迁移与退场文档，清理剩余的早期规划口径
2. 视需要补一份更短的“当前可用主链路清单”，方便快速交接
3. 在文档层基本收口后，再决定是否继续推进测试与可观测性收尾

### 2026-03-18 - 阶段 88：补齐当前完成态基线并同步重构进度百分比

#### 本阶段目标

- 把当前已经形成的“可用态”继续升级成一份更适合交接和汇报的完成态基线
- 不再只在长文档或提交历史里表达当前状态，而是提供一份简洁、直接、可复用的当前基线说明
- 将当前主线的重构进度百分比正式同步到仓库文档中

#### 已完成事项

1. 新增了当前本地完成态基线文档
   - 新增 `infra/transition/CURRENT_LOCAL_BASELINE.md`
   - 明确当前推荐运行时
   - 明确四个核心服务与统一入口
   - 明确当前主链路事实、验收标准、已退场模块与剩余治理项
   - 让后续交接时不再需要先翻完整的 `REFACTOR_PROGRESS.md`

2. 在根说明中补了完成态基线入口
   - 更新 `README.md`
   - 新增对 `infra/transition/CURRENT_LOCAL_BASELINE.md` 的指引
   - 让使用者从根文档就能快速跳到当前主线基线说明
...
4. 在重构执行记录顶部同步了当前进度
   - 更新本文档顶部“当前重构进度（2026-03-18）”
   - 让后续继续推进时，不需要每次重新推算“现在到底到多少了”

#### 当前结果

当前仓库除了已经具备统一运行入口、统一验收入口之外，还新增了：

- 一份更短、更适合交接的当前完成态基线文档
- 一份明确写出总进度与分项进度百分比的迁移状态说明
- 一份在执行记录顶部就能直接看到的当前完成度摘要

这意味着当前项目的“完成态”已经不仅可运行，也已经开始具备更像交付物的表达方式。

#### 这一步为什么重要

- 到了当前阶段，单纯说“差不多做完了”已经不够，需要一个仓库内可追踪的进度口径
- 把进度百分比与当前基线文档补上后，后续无论是你自己继续推进、还是别人接手，都能快速知道：
  - 当前已经完成了什么
  - 还剩什么
  - 剩余工作大概占多少
...
1. 继续收口测试与可观测性资产，把当前 `92%` 往“长期维护完成态”推进
2. 继续清理少量仍停留在历史规划口径的迁移文档
3. 视需要补充更明确的交付边界说明，区分“已可用”与“已治理完成”

### 2026-03-18 - 阶段 89：为统一运行脚本补齐单元测试并上调进度百分比

#### 本阶段目标

- 直接补齐当前完成态里最短板的一块：统一运行入口虽然已经可用，但还缺少自动化测试保护
- 让后续继续修改 `.tools/local_stack.py` 时，不会轻易把当前统一启动/停止/验收入口改坏
- 在测试资产真实增加后，再同步上调当前重构进度百分比

#### 已完成事项

1. 为统一本地栈脚本新增了单元测试
   - 新增 `.tools/test_local_stack.py`
   - 当前覆盖了：
     - `http_ok()` 的请求方式与异常兜底行为
     - `ensure_prerequisites()` 的前置条件提示
     - `main()` 对 `verify / up` 等命令路由的正确性
   - 让 `.tools/local_stack.py` 不再只是“人工手测通过”的脚本

2. 在根说明里补了测试命令入口
   - 更新 `README.md`
   - 新增：
     - `python .tools\test_local_stack.py`
   - 让后续维护者能直接在仓库根目录验证统一运行脚本没有被改坏

3. 同步上调了当前进度百分比
   - 更新 `MIGRATION_CHECKLIST.md`
   - 更新本文档顶部的“当前重构进度”
   - 当前将：
     - 测试与可观测性收尾：由 `55%` 上调到 `60%`
     - 总体完成度：由 `92%` 上调到 `93%`
...
- 统一运行脚本不仅能用，而且已经开始有自动化测试保护
- 根说明里已经直接给出了测试入口
- 当前进度百分比也不再只是基于口头判断，而是随着真实测试资产增加同步上调

这说明当前剩余的 `7%` 已经更明确地集中在业务测试、监控说明和少量历史资产持续收口上。

#### 这一步为什么重要

- 当前最容易在后续迭代中被悄悄改坏的，其实就是统一脚本和本地恢复入口
- 给这部分补上单元测试后，当前“可交接、可复现”的完成态会明显更稳
- 这也让当前进度从“运行链路已完成”继续向“维护基线更可靠”推进

#### 下一步计划

下一步优先考虑以下动作：

1. 继续补关键业务链路或验收脚本周边测试，把测试与可观测性收尾继续往上推
2. 继续清理剩余迁移文档里的早期规划口径
3. 视需要补一份更明确的“后续仅剩治理项”清单，让完成态边界更清楚

### 2026-03-18 - 阶段 90：为本地迁移验收脚本补齐自动化测试

#### 本阶段目标

- 继续把当前完成态从“统一运行入口有测试”推进到“统一验收入口也有测试”
- 降低后续修改 `.tools/verify_local_migration.py` 时把验收脚本悄悄改坏的风险
- 在新增真实测试资产后，再同步上调当前重构进度百分比

#### 已完成事项

1. 为本地迁移验收脚本新增了单元测试
   - 新增 `.tools/test_verify_local_migration.py`
   - 当前覆盖了：
     - `verify_nacos_service()` 对健康实例端口的判断
     - `verify_nacos_service()` 在空 `hosts` 场景下的失败行为
     - `verify_data_id()` 对空配置内容的拦截
     - `verify_http_check()` 对回测接口核心字段的约束
     - `main()` 在成功与失败路径上的输出与退出码
...
3. 真实执行并验证了新增测试
   - 实际执行 `python .tools\test_verify_local_migration.py`
   - 当前结果通过
   - 同时再次执行 `python .tools\local_stack.py verify`
   - 当前结果仍为 `VERIFICATION PASSED`
...
- 统一验收脚本也已有测试
- 根说明里已经直接给出两类测试入口
- 当前剩余的差距已经更明显地集中在业务测试、可观测性治理和少量文档尾项

这意味着当前 `94%` 的完成度，已经不只是“主链路可用”，而是开始具备更稳的交接与维护基线。

#### 这一步为什么重要

- 当前主线最关键的两个基础入口，一个是 `local_stack.py`，一个是 `verify_local_migration.py`
- 只有它们都受到自动化测试保护，当前“可运行 + 可验收”的完成态才足够稳
- 这一步也让后续继续提升测试覆盖时，能够从更关键、更高价值的入口开始向内推进

#### 下一步计划

下一步优先考虑以下动作：

1. 继续补关键业务服务或接口层测试，把当前 `94%` 往 `95%+` 推进
2. 继续清理少量仍停留在早期规划口径的迁移文档
3. 视需要补充统一可观测性说明，把运行、验收、监控三条口径继续收口

### 2026-03-18 - 阶段 91：在 README 收口运行顺序与功能说明

#### 本阶段目标

- 直接把“怎么运行项目、每个服务干什么”收口到根 README
- 不再要求使用者必须同时翻 `REFACTOR_PROGRESS.md`、迁移清单和对话记录才能上手
- 在根文档真正具备“可直接照着跑”的能力后，再同步上调当前重构进度百分比
...
4. 补齐了推荐使用方式
   - 在 README 中增加日常开发/调试的推荐节奏：
     - `up`
     - `status`
     - 打开页面
     - `verify`
     - `down`
   - 让根说明真正承担起“本地交接入口”的角色

5. 同步上调了当前进度百分比
   - 更新 `MIGRATION_CHECKLIST.md`
   - 更新本文档顶部“当前重构进度”
   - 当前将：
     - 本地运行/交接入口：由 `95%` 上调到 `98%`
     - 历史资产彻底清理：由 `85%` 上调到 `88%`
     - 总体完成度：由 `94%` 上调到 `95%`

#### 当前结果

当前根 README 已经不再只是“有一些零散命令”：

- 可以直接看到运行项目的顺序
- 可以直接知道每个核心服务负责什么
- 可以直接照着统一脚本把项目拉起来、验收、停掉

这意味着当前仓库的交接门槛又明显降了一层，也让当前 `95%` 的完成度更接近真正可交付的状态。

#### 这一步为什么重要

- 对当前阶段来说，最影响交接效率的已经不是代码能不能跑，而是文档能不能让别人快速上手
- 只有根 README 也达到“拿来即用”的水平，当前迁移完成态才算真正从开发者上下文里脱出来
- 这一步也让剩余的 `5%` 更清楚地集中在业务测试、可观测性说明和少量历史文档尾项

#### 下一步计划

下一步优先考虑以下动作：

1. 继续补关键业务服务或接口层测试，把当前 `95%` 往更稳的维护态推进
2. 继续补统一可观测性说明，把运行、验收、监控三条口径继续收口
3. 继续清理少量仍停留在早期规划口径的迁移文档

### 2026-03-18 - 阶段 92：补齐回测控制器测试并再次上调重构进度

#### 本阶段目标

- 继续把当前完成态从“运行入口和验收入口有测试”推进到“关键业务入口也有测试”
- 直接收口这次迁移中最容易回归的两个问题：
  - 空数据时不能直接炸成 `500`
  - 带尾斜杠的回测 URL 不能再次掉回 `404`
- 在关键业务测试真实落地后，再同步上调当前重构进度百分比

#### 已完成事项

1. 为回测控制器新增了 Web 层测试
   - 新增 `trend-trading-backtest-service/src/test/java/bupt/web/BackTestControllerTest.java`
   - 当前覆盖了两个关键场景：
     - 指数数据为空时，接口应返回结构化空结果而不是异常
     - 带尾斜杠的 `/simulate/.../` 路径仍能正常匹配并返回结果

2. 真实执行了回测服务模块测试
   - 实际执行：
     - `.tools\apache-maven-3.9.9\bin\mvn.cmd -pl trend-trading-backtest-service test`
   - 当前结果为 `BUILD SUCCESS`
   - 说明新增控制器测试与既有 `ResilientIndexDataGatewayTest` 一起通过，没有破坏当前回测主链路

3. 在 README 中补了业务侧测试入口
   - 更新 `README.md`
   - 新增回测服务测试命令说明
   - 明确当前这组测试已经覆盖：
     - 空数据场景不再直接抛错
...
5. 同步上调了当前进度百分比
   ...
   - 更新本文档顶部“当前重构进度”
   - 当前将：
     - 测试与可观测性收尾：由 `65%` 上调到 `72%`
     - 总体完成度：由 `95%` 上调到 `96%`

#### 当前结果

当前仓库在“完成态”上的稳健性又往前走了一步：
...
这意味着当前剩余的 `4%` 已经更聚焦在更深层的业务测试、统一可观测性说明与少量文档尾项上。

#### 这一步为什么重要

- 当前最关键的业务入口就是回测接口
- 之前真实踩过的两个问题，正好都落在这个控制器上
- 把这两个问题固化成自动化测试后，当前“100%可用态”的可靠性就明显更高了

#### 下一步计划

下一步优先考虑以下动作：

1. 继续补统一可观测性说明，把运行、验收、监控三条口径收口到根文档
2. 继续清理少量仍停留在早期规划口径的迁移文档
3. 视需要再补一组更靠业务结果的服务层测试，把当前 `96%` 继续往上推

### 2026-03-18 - 阶段 93：补齐统一可观测性说明并再次上调重构进度

#### 本阶段目标

- 把当前“主链路能跑、验收脚本能过”的完成态继续推进到“监控入口也有统一说明”
- 不再要求后续维护者自己去翻 `docker-compose.yml`、`prometheus.yml` 和 `Grafana provisioning` 文件才能知道当前监控样板怎么用
- 在统一可观测性说明真实补齐后，再同步上调当前重构进度百分比

#### 已完成事项

1. 在根 README 中补齐了监控与可观测性入口
   - 更新 `README.md`
   - 新增“监控与可观测性”小节
   - 明确当前推荐顺序是：
     - 先 `up`
     - 再 `verify`
     - 再按需启动 `Prometheus / Grafana`
   - 明确给出：
     - `Prometheus` 入口 `http://127.0.0.1:9090`
     - `Grafana` 入口 `http://127.0.0.1:3000`
     - 默认账号密码 `admin / admin`
   - 同时把当前主链路三个默认抓取目标直接写进了根文档

2. 为 Prometheus 样板新增了中文说明
   - 新增 `infra/docker-compose/prometheus/README.md`
   - 说明了：
     - 当前目录作用
     - 启动前提
...
3. 为 Grafana 样板新增了中文说明
   - 新增 `infra/docker-compose/grafana/README.md`
   - 说明了：
     - 当前目录作用
     - 与 Prometheus 的配合关系
     - 当前启动顺序
     - 默认入口与默认账号密码
     - 已预置的数据源与总览面板
     - `Trend Services Overview` 面板当前覆盖的三张状态卡片

4. 同步上调了当前进度百分比
   - 更新 `MIGRATION_CHECKLIST.md`
   - 更新本文档顶部“当前重构进度”
   - 当前将：
     - 测试与可观测性收尾：由 `72%` 上调到 `80%`
     - 总体完成度：由 `96%` 上调到 `97%`

#### 当前结果

当前仓库在“迁移完成态”上的表达方式又完整了一层：

- 运行入口有统一脚本
- 验收入口有统一脚本与测试
- 监控入口也已经有根文档指引和子目录中文说明

这意味着当前不再只是“服务能跑”，而是已经具备：
...
- 当前仓库已经补齐 `Prometheus` 和 `Grafana` 样板很久了，但如果没有中文说明，后续维护者仍然要靠读配置文件猜怎么使用
- 只有把“运行、验收、监控”三条入口都收口到文档层，当前迁移主线才更接近真正意义上的交付完成态
- 这一步也让剩余的 `3%` 更清晰地集中在：
  - 更深层的业务测试
  - 少量历史规划口径继续清理
  - 可观测性后续长期治理
...
1. 继续补一组更靠业务结果的服务层测试，把当前 `97%` 往 `98%+` 推
2. 继续清理少量仍停留在早期规划口径的迁移文档
3. 视需要补充更统一的调试/监控联动说明，让当前完成态更适合长期维护

### 2026-03-18 - 阶段 94：补齐回测服务层测试并继续上调重构进度

#### 本阶段目标

- 把当前测试覆盖继续从“统一入口 + 控制器入口”推进到“核心业务计算服务本身”
- 直接给 `BackTestService` 这层最关键的回测计算逻辑补上自动化保护
- 在服务层测试真实落地后，再同步上调当前重构进度百分比，并让根 README 跟进新的测试口径

#### 已完成事项

1. 为回测服务新增了业务层单元测试
   - 新增 `trend-trading-backtest-service/src/test/java/bupt/service/BackTestServiceTest.java`
   - 当前覆盖了：
     - `listIndexData()` 会对远端返回结果做倒序处理
     - `getYear()` 在空输入、空列表、单条数据场景下返回 `0`
     - `simulate()` 在空数据时返回稳定结构而不是异常
     - `simulate()` 在典型买卖场景下会产出：
       - `profits`
       - `trades`
       - `winCount / lossCount`
       - `annualProfits`

2. 让 README 跟进新的测试覆盖口径
   - 更新 `README.md`
   - 将回测模块测试说明从“控制器稳定性”扩展为“业务回归测试”
   - 明确当前回测模块测试已经覆盖：
     - 空数据保护
     - 尾斜杠 URL 兼容
     - 倒序处理
...
3. 同步上调了当前进度百分比
   ...
   - 更新本文档顶部“当前重构进度”
   - 当前将：
     - 测试与可观测性收尾：由 `80%` 上调到 `88%`
     - 总体完成度：由 `97%` 上调到 `98%`

#### 当前结果

当前仓库在“迁移完成态”的稳健性上又往前走了一步：
...
这意味着当前主线已经不只是“能跑、能验收、能看监控”，而是对最关键的业务计算路径也开始具备更实在的回归保护。

#### 这一步为什么重要

- 当前最值钱、也最容易在后续调整中被悄悄改坏的，就是回测服务自己的计算逻辑
- 只测控制器还不够，必须把服务层补上，当前“100% 可用态”才更有长期维护价值
- 这一步也让剩余的 `2%` 更清楚地集中在：
  - 少量历史规划口径继续清理
  - 更细粒度的业务测试继续补齐
  - 可观测性后续长期治理
...
1. 继续清理少量仍停留在早期规划口径的迁移文档，把历史资产彻底清理继续往上推
2. 视需要补更多贴近业务结果的数据/服务层测试，让当前 `98%` 再往上靠近完成态上限
3. 继续把 README 和迁移说明口径压到更适合交接和长期维护

### 2026-03-18 - 阶段 95：统一当前入口文档口径并正式收口到 100%

#### 本阶段目标

- 把当前仍残留在少量入口文档中的“迁移中 / 剩余 2% / 后续再切换”口吻彻底收口掉
- 明确区分：
  - 当前迁移主线已经完成
  - 后续仍可继续做的工作属于演进项，不再计入迁移完成度
- 让根 README、配置中心说明、完成态基线、迁移清单和退场说明在同一口径下闭环

#### 已完成事项

1. 收口了根 README 的当前状态表述
   - 更新 `README.md`
   - 明确当前仓库主线已经达到：
     - `100%` 迁移完成态
     - `100%` 本地可用态
   - 同时补上了 `infra/nacos-config/README.md` 的入口链接

2. 收口了 Nacos Config 说明文档
   - 更新 `infra/nacos-config/README.md`
   - 将原本“模板准备区 / 后续再接入”的表述改为当前真实状态：
     - `Nacos Config` 已成为默认配置中心
     - 核心 `Data ID` 已通过仓库脚本同步和验收
     - 这里现在承担的是“当前配置资产目录”角色，而不是只做迁移占位

3. 收口了当前完成态基线文档
   - 更新 `infra/transition/CURRENT_LOCAL_BASELINE.md`
   - 明确当前已经达到 `100%` 迁移完成态
   - 将后续工作统一下调为“演进项”，不再描述为迁移剩余项
...
4. 收口了迁移清单顶部进度口径
   ...
   - 明确：
     - 当前已经没有阻塞迁移完成态的剩余事项
     - 后续工作属于长期演进，不再计入迁移完成度

5. 为退场方案文档补上归档说明
   - 更新 `infra/transition/LEGACY_INFRA_RETIREMENT_PLAN.md`
   - 在文档顶部明确：
     - 本文主要保留为退场过程归档记录
     - 其中“试点 / 退场步骤 / 退场前提”描述的是历史过程，不代表当前仍未完成
...
- 如果只在口头上说“差不多做完了”，但入口文档还写着“还剩 2%”，那这个仓库在交接意义上就还没真正完成
- 只有把当前状态、完成度和后续工作的边界都写清楚，迁移完成态才算真正闭环
- 这一步也标志着当前任务从“重构迁移”正式转入“后续演进”

#### 下一步计划

当前迁移主线已经完成，本阶段不再登记迁移类下一步计划。
