# 服务迁移矩阵

## 目的

这份矩阵用于说明每个旧模块未来会如何迁移、保留、合并或退场。

## 当前迁移矩阵

| 当前模块 | 类型 | 当前状态 | 目标状态 | 处理方式 |
|---|---|---|---|---|
| `eureka-server` | 基础设施 | 已退场 | 删除 | 已从主构建移除且源码目录已删除 |
| `index-config-server` | 基础设施 | 已退场 | 删除 | 已从主构建移除且源码目录已删除 |
| `index-zuul-service` | 基础设施 | 已退场 | 删除 | 已由 `gateway-service` 接管且源码目录已删除 |
| `index-hystrix-dashboard` | 基础设施 | 已退场 | 删除 | 已从主构建移除且源码目录已删除 |
| `index-turbine` | 基础设施 | 已退场 | 删除 | 已从主构建移除且源码目录已删除 |
| `gateway-service` | 基础设施 | 新增 | 保留 | 新网关试点模块 |
| `third-part-index-data-project` | 业务 | 保留 | 待定 | 未来可能转为 fixture / mock-provider |
| `market-data-service` | 业务 | 新增 | 保留 | 市场数据读能力收敛试点模块 |
| `index-gather-store-service` | 业务 | 已退场 | 合并 | 同步能力已开始并入 `market-data-service`，旧模块源码目录已删除 |
| `index-codes-service` | 业务 | 已弱化 | 合并 | 新前端默认链路已切到 `market-data-service` |
| `index-data-service` | 业务 | 已弱化 | 合并 | 回测服务默认链路已切到 `market-data-service` |
| `trend-trading-backtest-service` | 业务 | 保留 | 保留 | 作为核心服务持续演进 |
| `trend-trading-backtest-view` | 前端/服务端混合 | 保留 | 删除 | 当前作为纯跳转壳层，旧页面资源已移除 |
| `trend-web` | 前端 | 新增 | 保留 | 已形成多页面应用壳层，继续接管旧视图入口 |

## 当前阶段结论

当前阶段已经进入“旧监控模块退场、旧注册/配置/网关模块持续退场”的状态：

- 新监控替代样板已入库
- `index-hystrix-dashboard` 与 `index-turbine` 已从主构建移除且源码目录已删除
- `eureka-server` 与 `index-config-server` 已从主构建移除且源码目录已删除
- `index-zuul-service` 已从主构建移除且源码目录已删除
- `market-data-service` 已作为市场数据收敛试点模块落库
- `index-gather-store-service` 已从主构建移除且源码目录已删除，旧同步职责开始收敛到 `market-data-service`
- `index-codes-service` 与 `index-data-service` 的默认入口地位已开始被 `market-data-service` 取代
- 其余旧模块仍按“并行迁移、逐步退场”继续推进
